...
impl<'a> Crypto for DpeCrypto<'a> {
    ...
    fn rand_bytes(&mut self, dst: &mut [u8]) -> Result<(), CryptoError> {
        ...
        while curr_idx < dst.len() {
            let trng_bytes = <[u8; 48]>::from(self.trng.generate().unwrap());
            let bytes_to_write = min(dst.len() - curr_idx, 48);
            dst[curr_idx..curr_idx + bytes_to_write].copy_from_slice(&trng_bytes);
